...
api.sendMessage(response.data['success'], event.threadID, event.messageID);
            ...
            if (data.length < 2) {
                ...
                try {
                    data.shift()
                    let txt = data.join(" ");
                axios.get(`https://manhict.tech/api/ytplay?query=${txt}&apikey=E8QAKPmf`)
                        .then(response => {
                        	api.sendMessage(`🔍Searching for '${txt}'`, event.threadID, event.messageID);
                       request(encodeURI(`${response.data.result.audio}`)).pipe(fs.createWriteStream(__dirname + '/cache/music.mp3')).on('finish',() =>{
                        var message = {
                                                body:`Here's your music, enjoyy!\n\n🎶Song Title » ${response.data.result.title}\n\nEnjoyy listening!`,
